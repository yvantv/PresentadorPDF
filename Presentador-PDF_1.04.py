...
class VentanaControl(QMainWindow):
    ...
    def abrir_pdf(self):
        """Verifica anotaciones pendientes antes de abrir un nuevo documento."""
        if self.doc_pdf and self.proyector.capa_anotaciones.tiene_anotaciones():
            ...
            if respuesta == QMessageBox.StandardButton.Save:
                ...
                # Si el usuario cancela la ventana de guardado, abortamos la apertura del nuevo PDF
                if not guardado_exitoso:
                    return
            elif respuesta == QMessageBox.StandardButton.Cancel:
                return  # Abortar la acción y permanecer en el PDF actual
        ...
        if path:
            ...
            self.pagina_actual = 0
            # Reiniciar todas las anotaciones existentes para el nuevo documento
    ...
    def guardar_pdf_anotado(self):
        """Redimensiona el PDF a 1920x1080 pt nativos y fusiona las trazas.
        Retorna True si el archivo se guardó correctamente, False si se canceló."""
        if not self.doc_pdf:
            return False
