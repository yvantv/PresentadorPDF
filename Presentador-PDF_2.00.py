...
class LayerAnotaciones(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        # Pestañas normales: 0..N-1 (índice de página del PDF)
        # Pizarras: "pizarra_1", "pizarra_2", etc.
...
class VentanaControl(QMainWindow):
    ...
    def __init__(self):
        ...
        self.ultima_pagina_pdf = 0  # Para recordar dónde estábamos antes de abrir pizarra
        
        self.pizarras = []  # Almacena los IDs de pizarras creadas: ["pizarra_1", "pizarra_2"]
        self.indice_pizarra_actual = -1  # -1 indica que estamos en el PDF

        self.proyector = VentanaProyeccion(ventana_control=self)
        self.settings = QSettings()

        # Historial de los 4 últimos colores elegidos
    ...
    def init_ui(self):
        ...
        toolbar_layout.addWidget(btn_board)
        toolbar_layout.addWidget(btn_new_board)
        toolbar_layout.addWidget(btn_pdf_mode)
        toolbar_layout.addStretch()
        toolbar_layout.addWidget(self.btn_pen)
        toolbar_layout.addWidget(self.btn_hl)
        toolbar_layout.addWidget(self.btn_color)

        # Paleta de 4 Colores Recientes
        ...
        toolbar_layout.addLayout(paleta_layout)
        toolbar_layout.addWidget(lbl_grosor)
        toolbar_layout.addWidget(self.spin_grosor)
        toolbar_layout.addWidget(self.btn_laser)
    ...
    def alternar_o_crear_pizarra(self):
        ...
        if not self.pizarras:
            ...
            if self.indice_pizarra_actual == -1:
                # Estábamos en el PDF -> Ir a la primera pizarra
                self.indice_pizarra_actual = 0
            else:
                # Rotar a la siguiente pizarra
    ...
    def next_page(self):
        if self.indice_pizarra_actual >= 0:
            ...
            if self.doc_pdf and self.pagina_actual < len(self.doc_pdf) - 1:
                ...
                # Si llegamos al final del PDF e intentamos avanzar, va a la pizarra
                self.indice_pizarra_actual = 0
                self.renderizar_vista()
    ...
    def prev_page(self):
        if self.indice_pizarra_actual >= 0:
            if self.indice_pizarra_actual > 0:
                ...
                # Salir de las pizarras y volver al PDF
                self.volver_al_pdf()
        else:
    ...
    def guardar_pdf_anotado(self):
        ...
        for id_pizarra in self.pizarras:
            # Crear una nueva página en el PDF con resolución Full HD (1920x1080)
