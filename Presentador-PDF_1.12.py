...
class LayerAnotaciones(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        # Habilitar rastreo continuo del ratón sin requerir clics
    ...
    def leaveEvent(self, event):
        """Oculta el láser y restaura el cursor cuando el puntero sale de la pantalla/cuadro."""
        if self.modo == 'laser':
            self.pos_laser = None
            self.laser_presionado = False
            self.unsetCursor()
    ...
    def enterEvent(self, event):
        """Oculta de nuevo el cursor estándar al volver a ingresar en modo láser."""
        if self.modo == 'laser':
            self.setCursor(Qt.CursorShape.BlankCursor)
    ...
    def mouseMoveEvent(self, event):
        ...
        if self.modo == 'laser':
            self.pos_laser = pos
            # Detectar si hay cualquier botón del mouse presionado (izquierdo o derecho)
    ...
    def paintEvent(self, event):
        ...
        # -----------------------------------------------------------
        # DIBUJO DEL LÁSER (Poco brilloso en movimiento, intenso al presionar)
        # -----------------------------------------------------------
        if self.modo == 'laser' and self.pos_laser:
            if self.laser_presionado:
                ...
                # POCO BRILLOSO / TRASLÚCIDO (Movimiento sin presionar)
