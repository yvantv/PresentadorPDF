...
class VentanaControl(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Panel de Control - Presentador PDF")
    ...
    def abrir_pdf(self):
        ...
        # Abre el diálogo en la última carpeta registrada
    ...
    def guardar_pdf_anotado(self):
        ...
        # Sugiere la última carpeta o el nombre del archivo actual con sufijo
        dir_inicial = self.obtener_directorio_inicial()
        if self.ruta_pdf_actual:
            nombre_sugerido = os.path.join(
                dir_inicial, 
                os.path.basename(self.ruta_pdf_actual).replace(".pdf", "_anotado.pdf")
            )
        else:
            nombre_sugerido = dir_inicial

        path_guardar, _ = QFileDialog.getSaveFileName(
            self, "Guardar PDF con Anotaciones", nombre_sugerido, "Archivos PDF (*.pdf)"
        )
        if not path_guardar:
            return False
