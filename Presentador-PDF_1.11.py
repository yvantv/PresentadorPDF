...
class LayerAnotaciones(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.trazo_actual = []
        self.pos_laser = None
        self.laser_presionado = False  # Para controlar la intensidad del láser
    ...
    def mouseMoveEvent(self, event):
        ...
        if self.modo == 'laser':
            self.pos_laser = pos
            # Detectar si se está presionando cualquier botón (izquierdo o derecho)
    ...
    def paintEvent(self, event):
        ...
        # -----------------------------------------------------------
        # DIBUJO DEL LÁSER (Dinámico según estado de presión)
        # -----------------------------------------------------------
        if self.modo == 'laser' and self.pos_laser:
            if self.laser_presionado:
                # BRILLANTE / INTENSO (Botón presionado)
                painter.setBrush(QColor(255, 0, 0, 230))  # Rojo muy opaco
                painter.setPen(Qt.PenStyle.NoPen)
                painter.drawEllipse(self.pos_laser, 14, 14)  # Algo más grande
                
                painter.setBrush(QColor(255, 255, 255, 255)) # Centro blanco puro
                painter.drawEllipse(self.pos_laser, 5, 5)
            else:
                # POCO BRILLOSO / ATENUADO (Botón sin presionar)
                painter.setBrush(QColor(255, 0, 0, 70))   # Rojo muy suave/transparente
                painter.setPen(Qt.PenStyle.NoPen)
                painter.drawEllipse(self.pos_laser, 10, 10)
                
                painter.setBrush(QColor(255, 255, 255, 120)) # Centro suave
...
class VentanaControl(QMainWindow):
    ...
    def actualizar_titulo_ventana(self):
        """Actualiza el título agregando el nombre del archivo si hay uno abierto."""
        if self.ruta_pdf_actual:
            nombre_archivo = os.path.basename(self.ruta_pdf_actual)
            self.setWindowTitle(f"{self.TITULO_BASE} - [{nombre_archivo}]")
        else:
            self.setWindowTitle(self.TITULO_BASE)
